...
import { Wine, Utensils, Sandwich } from "lucide-react"
...
export function FeatureSteps({
  ...
  const [speed] = useState(1) // 1x, 0.5x, 2x speed multipliers

  const getStepIcon = (index: number) => {
    const icons = [Wine, Utensils, Sandwich]
    const IconComponent = icons[index] || Wine
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      if (progress < 100) {
        setProgress((prev) => prev + (100 / (autoPlayInterval / 100)) * speed)
      } else {
        goToNext()
      }
    }, 100)
  ...
  }, [progress, features.length, autoPlayInterval, isPaused, speed, goToNext])
          ...
          <div className="order-2 md:order-1 space-y-8">
            {features.map((feature, index) => (
              ...
              >
                <motion.div
                  className={cn(
                    "w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center border-2 cursor-pointer",
                    index === currentFeature
                      ? "bg-primary border-primary text-primary-foreground scale-110"
                      : "bg-muted border-muted-foreground hover:border-primary/50",
                  )}
                  onClick={() => allowManualNavigation && goToStep(index)}
                >
                  {getStepIcon(index)}
                </motion.div>
